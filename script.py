...
def input_modifier(input, state):
    ...
    context_length = int(truncation_length)
    max_tokens = int((context_length - max_new_tokens - system_instruction_tokens) / 4)
    ...
    print(
        f"""
          ...
          Websearch settings:
            Max chars per search result: {max_characters_per_result}
    ...
    for result_url in links:
        ...
        if token_count >= max_tokens:
            print(
                f"breaking out early from web searches; token count: {token_count}, max_tokens: {max_tokens}"
            )
            break
    ...
    result_text = (
        "Given the following context, answer the user without making up facts.\n"
    )
...
def get_token_count(text):
    encoded_list: list[str] = shared.tokenizer.encode(str(text))
    return len(encoded_list)
